...
from ..utils.checks import admins_only, owners_only
...
class ExperimentalCog(BaseCog):
    ...
    async def _get_messages(
        ...
        async for message in channel.history(
            limit=limit, after=datetime(year=2016, month=2, day=15)
        ):
                    ...
                    for reaction in message.reactions:
                        ...
                        for user in u:
                            ...
                            if emoji.id not in users[user.id].reacts:
                                users[user.id].reacts[emoji.id] = Emoji(
    ...
    async def top_reacters(self, ctx: commands.Context) -> None:
        ...
        embed = await self.get_embed(
            ctx, title="Top Reacters", description="_", fields=fields
        )
