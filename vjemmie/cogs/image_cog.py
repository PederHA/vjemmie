...
class ImageCog(BaseCog):
    ...
    async def deepfry(self, ctx: commands.Context, *args, rtn=False) -> None:
        ...
        if (
            not ctx.message.attachments
            and not args
            or args
            and args[0] in ["help", "hlep", "?", "info"]
        ):
            out_str = (
        ...
        return await self._deepfry(
            ctx, url=a.url, emoji=a.emoji, text=a.text, caption=a.caption, rtn=rtn
        )
    ...
    async def remove_bg(
        self, ctx: commands.Context, image_url: Optional[str] = None
    ) -> None:
    ...
    async def _use_removebg_api(self, image: io.BytesIO) -> io.BytesIO:
        response = await post(
            "https://api.remove.bg/v1.0/removebg",
            files={"image_file": image},
            data={"size": "auto"},
            headers={"X-Api-Key": secrets.REMOVEBG_KEY},
        )
    ...
    async def scale_to_target(
        self, width: int, height: int, target: int
    ) -> Tuple[int, int]:
    ...
    def resize_image(
        self, image: Image.Image, width: int = 0, height: int = 0
    ) -> Image.Image:
    ...
    async def img_to_txt(self, ctx: commands.Context, url: str = None) -> None:
        ...
        if not url and not ctx.message.attachments:
            raise CommandError(
                "An image URL or an image message attachment is required!"
            )
        ...
        try:
            image_text = await self.bot.loop.run_in_executor(
                None, self.read_image_text, img
            )
        except pytesseract.pytesseract.TesseractNotFoundError:
            await self.warn_owner("Tesseract is not installed or is not added to PATH!")
            raise CommandError(
                "This command has not been properly configured by the bot owner yet."
            )
