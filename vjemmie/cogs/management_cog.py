...
class ManagementCog(BaseCog):
    ...
    async def react_messages(self, ctx) -> None:
        last_hour = datetime.now() - timedelta(
            hours=2
        )  # Some timezone fuckery means we have to do 2 here
        ...
        for guild in self.bot.guilds:
            for channel in guild.text_channels:
                async for msg in channel.history(after=last_hour):
                    if msg.reactions:
                        reactions = [
                            reaction for reaction in msg.reactions if reaction.count > 1
                        ]
                        if len(reactions) >= 3:
    ...
    async def _dump_cogs(self, enabled, disabled) -> None:
        out_dict = {"enabled": enabled, "disabled": disabled}
