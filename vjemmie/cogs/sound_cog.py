...
from ..utils.converters import URLConverter
...
ytdlopts = {
    ...
    "outtmpl": f"{str(config.YTDL_DIR.path)}/%(title)s.%(ext)s",
    "noplaylist": True,
...
ffmpegopts = {
    ...
    "options": f"-vn -loglevel {config.FFMPEG_LOGLEVEL}",
}
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_local_source(
        cls, ctx: commands.Context, subdir: str, filename: str
    ):
...
class SoundDirectory:
    ...
    def __init__(
        ...
        self.color = color
        self.cached_at = 0.0
        self._sound_list: Dict[str, Path] = {}
...
class SoundCog(BaseCog):
    ...
    DIRS = [d.path for d in config.SOUND_SUB_DIRS]
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.sub_dirs = [
            ...
            for subdir in config.SOUND_SUB_DIRS
        ]
        ...
        self.played_count: DefaultDict[int, int] = defaultdict(
            int
        )  # Key: Guild ID. Value: n times played
    ...
    def sound_list(self) -> dict:
        ...
        for sd in self.sub_dirs:
            ...
            for k in list(sl.keys()):
                if k in sound_list:
                    new_name = self.get_unique_filename(
                        k, ext_sound_list={**sound_list, **sl}
                    )
                    self._do_rename_file(sd.path, k, new_name)
                    sound_list[new_name] = sl.pop(k)
                    logger.info(
                        f"{sd.path}/{k} has been renamed to {sd.path}/{new_name}"
                    )
    ...
    async def show_players(self, ctx: commands.Context) -> None:
        ...
        players = "\n".join([f"{str(self.bot.get_guild(gid))}" for gid in self.players])
        if players:
    ...
    async def times_played_session(self, ctx: commands.Context) -> None:
        ...
        """
        out = "\n".join(
            [f"{self.bot.get_guild(k)}: {v}" for k, v in self.played_count.items()]
        )
    ...
    async def connect(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if not vc and any(self.bot.user.id == user.id for user in channel.members):
            ...
            await ctx.invoke(
                self.stop
            )  # Immediately issue !stop command, removing bot user from channel
    ...
    async def play_local_source(
        self, ctx: commands.Context, player: AudioPlayer, sound_name: str
    ) -> None:
    ...
    async def play_ytdl_source(
        self, ctx: commands.Context, player: AudioPlayer, url: str
    ) -> None:
        ...
        if ctx.invoked_with == "ytdl":
            await self.check_downloads_permissions(
                add_msg="Use **`!play`** or **`!yt`** instead."
            )
            download = True
        else:
            download = False
        source = await YTDLSource.create_source(
            ctx, url, loop=self.bot.loop, download=download
        )
        await player.queue.put(source)
    ...
    async def _play(
        ...
        ctx: commands.Context,
        arg: str,
        voice_channel: commands.VoiceChannelConverter = None,
    ) -> None:
    ...
    @commands.command(
        name="yt",
        aliases=["ytdl", "spotify"],
        usage="<url>, <search query> or <Spotify URI>",
    )
    async def yt(self, ctx: commands.Context, *args):
        """Play YouTube or Spotify content."""
        arg = " ".join(args)
        youtube_client = await get_youtube_client(ctx)

        if "spotify" in arg:
            spotify_client = await get_spotify_client(ctx)
            await ctx.send("Attempting to find song on YouTube...", delete_after=5.0)
            (
                artist,  # thanks Black...
                song,
                _,
            ) = await spotify_client.get_song_info(arg)
            arg = await youtube_client.get_top_result(f"{artist} {song}")

        elif urlparse(arg).scheme not in ["http", "https"]:
            await ctx.send(f"Searching YouTube for `{arg}`...", delete_after=5.0)
            arg = await youtube_client.get_top_result(arg)

        await self._play(ctx, arg)

    @commands.command(name="rplay", usage="<channel_id>")
    async def remoteplay(
        self, ctx: commands.Context, channel: commands.VoiceChannelConverter, *args
    ) -> None:
    ...
    async def skip(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send(
                "I am not currently playing anything!", delete_after=5
            )
    ...
    async def change_volume(self, ctx, *, vol: int):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send(
                "I am not currently connected to voice!", delete_after=5
            )
    ...
    @commands.command(
        name="soundlist",
        aliases=["sounds"],
        description="Prints a list of all sounds on the soundboard.",
    )
    @commands.cooldown(rate=1, per=10, type=commands.BucketType.user)
    async def soundlist(
        self, ctx: commands.Context, category: Optional[str] = None
    ) -> None:
        ...
        if not category:
            await self.send_embed_message(
                ctx, title="Categories", description=categories
            )
        ...
        if len(_out) > config.SOUNDLIST_FILE_LIMIT:
            msg = (
        ...
        if not out:
            return await ctx.send(f"Category **`{category}`** is empty!")
    ...
    async def search(
        self, ctx: commands.Context, *query: str, rtn: bool = False
        ...
        embeds = await self._do_search(q, ctx)
        ...
        if len(embeds) > len(self.sub_dirs):
            ...
            raise CommandError(
                f"Search returned {n_results} results. A more specific search query is required."
            )
    ...
    async def _do_search(
        self, query: str, ctx: commands.Context = None
    ) -> List[discord.Embed]:
        ...
        for sf in self.sub_dirs:
            ...
            if _out:
                _out_str = "\n".join(_out)
                _rtn_embeds = await self.send_embed_message(
                    ctx, sf.header, _out_str, color=sf.color, return_embeds=True
                )
    ...
    async def queue(self, ctx: commands.Context) -> None:
        ...
        if not vc or not vc.is_connected():
            return await ctx.send(
                "I am not currently connected to a voice channel!", delete_after=5
            )
        ...
        out_msg = "\n".join(
            f"{idx}. **`{up['title']}`**" for idx, up in enumerate(upcoming, 1)
        )
    ...
    @commands.command(name="tts", aliases=["texttospeech", "text-to-speech"])
    async def texttospeech(
        ...
        if not text:
            return await self.send_error_msg(
                ctx, "Text for TTS is a required argument."
            )
            ...
            _langs = [
                ...
                f"{lang_long}"
                for lang_short, lang_long in valid_langs.items()
            ]
            langs = "\n".join(_langs)
    ...
    async def _do_create_tts_file(
        self,
        text: str,
        language: str,
        filename: str,
        *,
        directory: str = config.TTS_DIR,
    ...
    async def add_sound(
        self, ctx: commands.Context, url: str = None, filename: str = None
    ) -> None:
        ...
        if not ctx.message.attachments and not url:
            return await self.send_error_msg(
                ctx, "A file attachment or file URL is required!"
            )
        ...
        try:
            ...
            raise CommandError(f"Invalid file type. Must be one of: **{FILETYPES}**")
    ...
    async def _do_download_sound(
        self, ctx: commands.Context, url: str, *, filename: str = None
    ) -> str:
        ...
        if (
            not check_file_audio(sound_file) and ext != ".mp3"
        ):  # mp3 files can't always be identified by MIME type
            raise CommandError("Downloaded file does not appear to be an audio file!")

        filename = self.get_unique_filename(filename)
        filepath = f"{config.DOWNLOADS_DIR}/{filename}{ext}"
    ...
    async def rename_sound(
        self, ctx: commands.Context, original: str, new: str
    ) -> None:
        ...
        if new in self.sound_list:
            ...
            raise CommandError(
                "New filename cannot be identical to the original filename."
            )
        elif not directory:
    ...
    async def dl(self, ctx: commands.Context, url: URLConverter = None) -> None:
        ...
        if (
            ...
            or url
            and any(
                url.path.lower().endswith(filetype) for filetype in VALID_FILE_TYPES
            )
        ):
    ...
    @admins_only()
    async def join_sound_files(
        self, ctx: commands.Context, file_1: str, file_2: str
    ) -> None:
        ...
        for f in [file_1, file_2]:
            for p in Path(f"{config.SOUND_DIR}").glob(f"*/*{f}*"):
        ...
        await ctx.send(
            f"Combined **{file_1}** & **{file_2}**! New sound: **{joined.stem}**"
        )
