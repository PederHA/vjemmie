...
class WoWProgressCog(BaseCog):
    ...
    async def wowprogress(self, ctx: commands.Context, *args: str):
        try:
            ...
            # Raids Per Week
            raids_per_week = soup.find("div", class_="raids_week").string.split(
                ": ", 1
            )[1]
            ...
            # Guild name
            guild_name = (
                soup.find("title").string.partition("@")[0].split("Guild ", 1)[1]
            )

            # Progression
            progress = (
                soup.find("span", class_="innerLink ratingProgress").find("b").string
            )
