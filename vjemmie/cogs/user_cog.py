...
class UserCog(BaseCog):
    ...
    @commands.command(
        name="help", aliases=["Help", "hlep", "?", "pls"], usage="<command/category>"
    )
    async def help_(
        self,
        ctx: commands.Context,
        cmd_or_category: str = None,
        advanced: BoolConverter(["advanced"]) = False,
    ) -> None:
    ...
    async def help_command(
        ...
        top_users = self.bot.get_cog("StatsCog").get_top_command_users(
            ctx.guild.id, command, limit=10
        )
        if top_users:
    ...
    async def about(self, ctx: commands.Context) -> None:
        ...
        fields = [
            EmbedField(name="Owner", value=config.AUTHOR_MENTION),
            EmbedField(
                name="Running on",
                value=f"<:python:570331647933677590> Python {sys.version.split(' ')[0]}",
            ),
            EmbedField(name="Command categories", value=len(await self.get_cogs())),
            EmbedField(
                name="Useful commands",
                value=f"`{p}help`, `{p}commands`, `{p}categories`, `{p}changelog`",
            ),
            ...
            EmbedField(
                name="Uptime",
                value=self.bot.get_cog("StatsCog").get_bot_uptime(type=str),
            ),
        ]
    ...
    async def get_cooldowns(self, ctx: commands.Context) -> None:
        ...
        cooldowns = await self.bot.loop.run_in_executor(
            None, self._get_command_cooldowns, ctx
        )
