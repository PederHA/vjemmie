...
class NonCaseSensMemberConverter(IDConverter):
    ...
    async def convert(self, ctx, argument):
        ...
        def get_from_guild(
            guild: discord.Guild, argument: Union[str, int]
        ) -> Optional[discord.Member]:
            for member in guild.members:
                if (
                    member.name.lower() == argument
                    or member.nick
                    and member.nick.lower() == argument
                ):
                    return member
...
class URLConverter(commands.Converter):
    ...
    async def convert(
        ...
        if self.EXTENSIONS and not any(
            url.path.lower().endswith(ext) for ext in self.EXTENSIONS
        ):
            if self.category:
...
class BoolConverter(commands.Converter):
    ...
    async def convert(self, ctx: commands.Context, arg: Union[bool, str]) -> bool:
        ...
        if isinstance(arg, str):
            arg = arg.lower()
            return arg in self.options + config.YES_ARGS
...
class IPAddressConverter(commands.Converter):
    """Accepts a string and parses it as an ip address. Returns IPv4Address or IPv6Address."""
