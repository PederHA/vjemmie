...
SENTINEL = (
    object()
)  # Shouldn't strictly be called "sentinel", but it's not a None-value either...
...
def decorator_factory(
    attr_k: str, attr_v: Any = SENTINEL
) -> Any:  # Callable[[Any], Awaitable[[Any], Any]] IDK if this is correct, so we're going with Any
...
class TestCog(BaseCog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.network_io_enabled = (
            False  # Enables commands such as: !meme, !reddit get, !twitter add
        )
    ...
    async def _toggle_attr(
        self,
        ctx,
        attr: str,
        description: str,
        addendum: str = None,
        addendum_on_false: bool = False,
    ) -> None:
        # Toggle attr
    ...
    async def _do_test(self, coro_or_cmd, *args, **kwargs) -> None:
        ...
        cmd_name, _args, _kwargs = await self._get_test_attrs(
            coro_or_cmd, *args, **kwargs
        )
    ...
    async def _format_assertion_error(
        self, result: Any, cmd_name: str, assertion: Any
    ) -> None:
    ...
    async def do_test_command(
        self, ctx: commands.Context, cmd: commands.Command, *args, **kwargs
    ) -> None:
    ...
    async def do_test_cog_method(
        self, cog_name: str, meth_name: str, *args, **kwargs
    ) -> None:
    ...
    async def test_soundcog_yt(self, ctx: commands.Context) -> None:
        await self.do_test_command(
            ctx, "play", "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
        )
    ...
    async def test_statscog_get_top_commands_for_guild(
        self, ctx: commands.Context
    ) -> None:
        await self.do_test_cog_method(
            "StatsCog", "get_top_commands_for_guild", guild_id=ctx.guild.id
        )

    async def test_statscog_get_top_commands_for_user(
        self, ctx: commands.Context
    ) -> None:
        await self.do_test_cog_method(
            "StatsCog",
            "get_top_commands_for_user",
    ...
    async def test_twittercog_add(self, ctx: commands.Context) -> None:
        ...
        if username in twitter_cog.users:
            await self.do_test_command(
                ctx, "twitter add", username, raises=CommandError
            )
        else:
